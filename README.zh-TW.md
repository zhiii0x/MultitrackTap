...
**尚在 pre-release,但已可日常使用**——錄音引擎、UI、Reaper 匯出都已端到端可運作,並以
已簽章與公證的 DMG 發佈。

## 安裝

1. 從 [**最新 release**](https://github.com/zhiii0x/MultitrackTap/releases/latest) 下載 **Multitrack Tap.dmg**。
2. 打開 DMG,把 **Multitrack Tap 拖進「應用程式」資料夾**。
3. 開啟它。第一次使用時,到「系統設定 → 隱私權與安全性」授予**麥克風**與**系統音訊錄製**權限,
   再**結束並重新打開一次**,讓 macOS 套用系統音訊錄製授權。

已用 Developer ID 簽章 + 公證,開啟時不會有 Gatekeeper 警告。想自己建置?見下方。

## 從原始碼建置

需要 **Xcode 16 以上**(Swift 6)。
